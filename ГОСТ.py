...
def main(argv=None):
    if len(sys.argv) != 4:
        print(" 4 аргументті жаз !!! Сізде 4 аргументке жетпейді ")
        return
    if sys.argv[1] != '-s' and sys.argv[1] != '-d':
        print(" Шифрлау және дешифрлау үшін -d және -s опцияларын енгіз ")
    ...
    if sys.argv[1] == '-s':
        ...
        try:
            ...
            print(f" Файлды ашу мүмкін емес {sys.argv[2]}")
            return
        try:
            with open(sys.argv[3], 'w') as file:
                print(*cyphred, file=file)
            print(" Шифрланған файл ")
        except:
            print(f" Файлды ашу мүмкін емес {sys.argv[3]}")
            return
    if sys.argv[1] == '-d':
        decyphred = []
        gost = Crypt(key, blocks)
